...
export default function DirectorDetailPage() {
  ...
  if (!person) return <p className="p-6">Director no encontrado.</p>;

  const visibleMovies = showAll
    ? person.filmographyAsDirector
    : person.filmographyAsDirector.slice(0, FEATURED_MOVIES_LIMIT);
  ...
  return (
    <article className="flex flex-col gap-10 py-15 px-6">
      ...
      {person.filmographyAsDirector.length > 0 && (
        <section className="flex flex-col gap-8">
          <h2 className="text-3xl font-bold">Películas dirigidas</h2>
          <MovieGrid movies={visibleMovies} />

          {!showAll &&
            person.filmographyAsDirector.length > FEATURED_MOVIES_LIMIT && (
